...
export default function SalesReportPage() {
  ...
  const [endDate, setEndDate] = useState(""); // هنا سنختار تاريخ النهاية

  const fetchSalesReport = async () => {
    if (!endDate) return; // لا شيء يتم بدون تحديد النهاية
    try {
      setLoading(true);
      const token = localStorage.getItem("auth_token");

      const res = await fetch(
        `https://prog2025.goldyol.com/api/reports/sales?period=monthly&start_date=2000-01-01&end_date=${endDate}`,
  ...
  // جلب البيانات عند تغيير تاريخ النهاية
  useEffect(() => {
    fetchSalesReport();
  }, [endDate]);
  ...
  return (
    <div className="sales-report-page">
      ...
      <div className="end-date-selector">
        <label>اختر تاريخ النهاية: </label>
        <input
          type="date"
          value={endDate}
